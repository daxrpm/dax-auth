...
def create_faces_encoding_file():
    ...
    for face_index in range(NUM_FACES):
        ...
        try:
            ...
            logging.info(f"Encoding for face {face_index} saved successfully.")
        except Exception as e:
            logging.error(f"An error occurred while processing {
                          image_path}: {e}")
...
def read_faces_encoding_file():
    ...
    for face_index in range(NUM_FACES):
        ...
        try:
            ...
            logging.info(f"Encoding for face {
                         face_index} loaded successfully.")
        except FileNotFoundError:
            logging.warning(f"Encoding file {encoding_path} not found.")
        except Exception as e:
            logging.error(f"An error occurred while reading {
                          encoding_path}: {e}")
    return encodings
